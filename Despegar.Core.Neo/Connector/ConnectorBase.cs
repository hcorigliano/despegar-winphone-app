        ...
        protected async Task<T> ProcessRequest<T>(HttpRequestMessage httpMessage, ServiceKey key) where T : class
        {
            ...
            try
            {
                ...
                if (!requestSuccess)
                {
                    ...
                    {
                        e = new HTTPStatusErrorException(String.Format("[Core:Connector]: HTTP Error code {0} ({1}) Message: {2}", (int)httpResponse.StatusCode, httpResponse.StatusCode.ToString(), response), ex);                        
                    }
            ...
            {
                // HTTP Client error
                var e = new WebConnectivityException(String.Format("[Core:Connector]: Could not connect to Service URL ", httpMessage.RequestUri), ex);
                logger.LogException(e);
                throw e;
            }
            catch (JsonException ex)
            {
                ...
                var e = new JsonSerializerException(String.Format("[Core:Connector]: Service call: {0}. Could not deserialize type '{1}' from service response data: {2}", httpMessage.RequestUri, typeof(T).FullName, response), ex);
                logger.LogException(e);
                throw e;
            }
            catch (Exception ex) 
            {
                if (customExceptionThrown)                 
                  ...
                  var e = new Exception(String.Format("[Core:Connector]: Unknown Connector Error when calling Service URL {0}, Exception Message: {1}", httpMessage.RequestUri, ex.ToString()), ex);
                  logger.LogException(e);
                  throw e;
                }
          }
